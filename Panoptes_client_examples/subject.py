...
class Subject(PanoptesObject):
    ...
    def add_location(self, location):
        ...
        """
        if type(location) is dict:  # for externally hosted subjects
            self.locations.append(location)
            self._media_files.append(None)
            return

        elif type(location) in (str,) + _OLD_STR_TYPES:  # for local files
            f = open(location, 'rb')
            MEDIA_TYPE_DETECTION = 'mimetype'
        else:
            f = location  # for open file like objects
            MEDIA_TYPE_DETECTION = 'imghdr'

        try:
            media_data = f.read()
            if MEDIA_TYPE_DETECTION == 'mimetype':
                media_type = mimetypes.MimeTypes().guess_type(location, strict=True)[0]
            else:
