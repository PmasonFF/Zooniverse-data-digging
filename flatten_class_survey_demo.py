                            ...
                            try:
                                ...
                                for species in task['value']:
                                    ...
                                    writer.writerow({'classification_id': row['classification_id'],
                                                     ...
                                                     'any_horns': answer[3],
                                                     "don't_care": answer[4],
                                                     'subject_species': row['subject_ids'] + choice,
...
with open(aggregate_location, 'w', newline='') as ag_file:
    ...
    with open(sorted_location) as so_file:
        ...
        for row2 in sorted_file:
            ...
            new_subject_species = row2['subject_species']
            all_choices = json.loads(row2['all_choices'])
            if new_subject_species != subject_species:
                ...
                subject = new_subject
                subject_species = new_subject_species
...
2)  Then calculate the total v_f for a species as the sum of the vote fractions for any number
...
def apply_tests(sub, cl_tot, sp_num, col_head):
    ...
    sorted_species = sorted(sp_num, key=operator.itemgetter(1), reverse=True)
    if sorted_species[0][1] < 20:
        generate_row(sub, cl_tot[sub], 'indeterminate species, no consensus', '',
                     col_head, ['' for rc in range(0, len(col_head))])
        return

    if len(sp_num) > 1:
        # Apply test 3 - are all classifications single species?
        if cl_tot[sub][0] == cl_tot[sub][1]:
            if sorted_species[0][1] - sorted_species[1][1] >= 45:
                generate_row(sub, cl_tot[sub], sorted_species[0][0], sorted_species[0][1],
                             col_head, generate_outlist(sorted_species))
                return
            else:
                generate_row(sub, cl_tot[sub], 'indeterminate species', '',
                             col_head, ['' for rc in range(0, len(col_head))])
                return
        # Apply test 4 - multiple species classified, strong consensus for one species
        else:
            if sorted_species[0][1] - sorted_species[1][1] >= 45:
                generate_row(sub, cl_tot[sub], sorted_species[0][0], sorted_species[0][1],
                             col_head, generate_outlist(sorted_species))
                ...
                # Apply test 4 - multiple species classified, strong consensus for
                # multiple species
                if sorted_species[1][1] >= 65:
                    ...
                    generate_row(sub, cl_tot[sub], 'indeterminate species possibly multiple', '',
                                 col_head, ['' for rc in range(0, len(col_head))])
                    return
    else:
        generate_row(sub, cl_tot[sub], sp_num[0][0], sp_num[0][1],
                     col_head, generate_outlist(sp_num))
        return
...
def total_animals(sp_num):
    tot_animals = 0
    for r8 in range(0, len(sp_num)):
        for r9 in range(0, len(responses[0]) - 2):
            tot_animals += int(sp_num[r8][2][0][r9]) / 100 * int(responses[0][r9])
    return [int(round(tot_animals)), int(round((abs(tot_animals - int(tot_animals) - .5) + .5) * 100))]


def generate_outlist(sp_num, flg=False):
    # deal with the how many question
    # generate the bin_v_f list for the first species in sp_num
    bin_v_f = []
    tot_v_f = 0
    for r4 in range(0, len(responses[0])):
        if sp_num[0][2][0][r4] > 0:
            bin_v_f.append((int(responses[0][r4]), sp_num[0][2][0][r4]))
            tot_v_f += int(sp_num[0][2][0][r4])
    if len(bin_v_f) > 1:  # multiple bins (ie counting errors or some misidentification occurred)
        ...
        if binvf[0][1] >= binvf[1][1] + 45:  # good consensus for lowest bin:
            ...
            if binvf[0][1] >= binvf[1][1] + 45:  # strong consensus for highest bin:
                ...
                if flg:  # multiple species so total animals have a unknown split
                    out_list[0] = 'indeterminate count, multiple species'
                    out_list[1] = ''
                else:
                    out_list[0] = total_animals(sp_num)[0]
                    out_list[1] = total_animals(sp_num)[1]
    else:
        ...
        for row3 in aggregated_file:
            ...
            if new_subject != subject:
                if rc6 != 1:  # don't want to look at the empty initial values
                    apply_tests(subject, class_totals, species_number,
                                column_headers)
                subject = new_subject
                total_v_f = 0
                for r10 in range(0, len(responses[0])):
                    total_v_f += vector[0][r10]
                species_number = [(row3['choice'], total_v_f, vector)]
            else:
                total_v_f = 0
                for r10 in range(0, len(responses[0])):
                    total_v_f += vector[0][r10]
                species_number.append((row3['choice'], total_v_f, vector))
                subject = new_subject

        # catch the last aggregate after the end of the file is reached
        apply_tests(subject, class_totals, species_number, column_headers)
