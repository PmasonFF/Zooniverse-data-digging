...
your password, and also provide the Path and filename of the data export to use in line 36.  If this is
not used, the Path and filename of the data export can be hardcoded in line 36.
...
# if the run_config option is to be used the script run_config.py must be in the working directory.
...
metadata_fields = ['#details', '#Site', '#camera', 'image_1', '#datetime_1', 'image_2', '#datetime_2',
                   'image_3', '#datetime_3', 'image_4', '#datetime_4', 'image_5', '#datetime_5',
                   'image_6', '#datetime_6', 'image_7', '#datetime_7', 'image_8', '#datetime_8']
...
question_headers = ['How_many', 'Interesting_photo']
...
# questions are numbered from 0 in the order they are in the questions file.
custom_headers = {1: ['Not_interesting', 'Mangy', 'High_quality', 'Interacting', 'Carrying prey',
                      'Young_present', 'Tagged', ]}
...
# Optional - advance selected subjects to new subject set(s) for further classification or review.  For each group
# to be advanced, first give the subject set name to be used, then a list of the choice(s)to be selected - all subjects
# with ANY votes for these choices will be added to the subject set.  Note the "choices" listed can be just part of
# more than one actual choice and are case insensitive  - ie "Rabbit" will select for both "Cottontail rabbit" and
# "Brush rabbit". ['Rabbit', 'Squir'] will select for all subjects with votes for any choice with either of those
# character strings in them regardless of the case of the characters.
# The special case ["Indeterminate"] will select for all subjects that did not resolve to a consensus for some choice.
# example:  subjects_to_advance =[['Indeterminate_18308', ['Indeterminate']], ['Rabbits_18308', ['Rabbit']]]

subjects_to_advance = [['Indeterminate_2b_18308', ['Indeterminate']],
                       ['Rabbits_18308', ['Rabbit']]]

# ___________________________________________________________________________________________________________________

with open(directory + os.sep + 'Problem_subjects_data.csv')as p_file:
    problems = csv.DictReader(p_file)
    prob_subjects = []
    for prob_sub in problems:
        prob_subjects.append(int(prob_sub['zoo_subject']))
    print(len(prob_subjects))
...
out_location = directory + os.sep + out_file_name + '_flatten.csv'  # deleted after sorting
...
# set up sets of selected subjects to advance, convert to "zooniverse" choices format
sets_to_advance = {}
build_unresolved = ''
translate_table = dict((ord(char), '') for char in r'!"#%\'()*+,-./: <=>?@[\]^_`{|}~')
to_advance = []
for subjectset in subjects_to_advance:
    if subjectset[1] == ['Indeterminate']:
        build_unresolved = subjectset[0]
    else:
        sets_to_advance[subjectset[0]] = set()
        for index, item in enumerate(subjectset[1]):
            subjectset[1][index] = item.upper().translate(translate_table)
        to_advance.append(subjectset)
...
def include(class_record):
    ...
    # else:
    #     return False
    if 80000000 >= int(class_record['subject_ids']) >= 75131697:
        ...
        return False
    if int(class_record['subject_ids']) in prob_subjects:
        return False
    else:
        pass
...
def select_subjects(sub_, choice_, sets_to_advance_):
    for group_ in to_advance:
        for key_ in group_[1]:
            if choice_.find(key_) >= 0:
                sets_to_advance_[group_[0]] |= {sub_}


def build_subject_set(setname, subject_list):
    try:
        # check if the subject set already exits
        subject_set = SubjectSet.where(project_id=project.id, display_name=setname).next()
        ...
        #  subject_set.save()
    print(len(subject_list), ' will be added to subject set: {}'.format(setname))

    k = 0
    #  iterate through the subjects to advance verifying they load (for now) may use a list later.
    for subjet in subject_list:
        try:
            #  subject_set.add(subjet)  # Uncomment this line to activate subject_set additions
            k += 1
            print(subjet, 'linked')
...
with open(out_location, 'w', newline='', encoding='utf-8') as ou_file:
    fieldnames = ['classification_id', 'subject_ids', 'created_at', 'user_name', 'subject_choices', 'choice']
...
with open(aggregate_location, 'w', newline='') as ag_file:
    ...
    writer.writeheader()
    #  build a look-up table of classification totals by subject - this is needed for the calculation of vote_fraction.
        ...
        for row2 in sorted_file:
            ...
            if new_subject_choices != subject_choices:
                if row_counter != 1:  # don't want to output the empty initial values
                    ...
                    select_subjects(subject, choice_now, sets_to_advance)
...
# Optional steps to advance subjects to additional subject sets if requested in line 90

# This first section takes a list subjects with unresolved issues from the filter above and sets up a
...
if build_unresolved:
    print('To proceed to link', len(subjects_to_resolve), 'unresolved subjects to a subject set, enter "y"')
    link = input('any other entry to continue' + '\n')
    if link.lower() == 'y':
        build_subject_set(build_unresolved, subjects_to_resolve)

# This section adds subjects with specified choices to additional subject sets as set up in line 90
link = input('To proceed to link selected subjects to additional subject set(s), enter "y"' + '\n')
if link.lower() != 'y':
    quit()
for key in sets_to_advance.keys():
    build_subject_set(key, sorted(sets_to_advance[key]))
